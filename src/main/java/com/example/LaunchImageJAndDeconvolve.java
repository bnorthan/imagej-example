...
public class LaunchImageJAndDeconvolve {
	...
	public static <T extends RealType<T> & NativeType<T>> void main(final String[] args)
		...
		psf32 = (Img<FloatType>) ij.op().math().divide(psf32, new FloatType(ij.op().stats().sum(psf32).getRealFloat()));

		final long startTime = System.currentTimeMillis();

		final Img<FloatType> deconvolved = (Img<FloatType>) ij.op().deconvolve().richardsonLucy(img32, psf32,
				new long[] { 0, 0, 0 }, null, null, null, null, 5, true, true);
