...
public class ProjectionTest {
		...
		final String[] args) throws InterruptedException, IOException
	{
		...
		for (d = 0; d < currentData.numDimensions(); ++d) {
			if (d != dim) projected_dimensions[d] = (int) currentData.dimension(d);
		}
		...
		// UnaryComputerOp mean_op =Computers.unary(ij.op(), Ops.Stats.Mean.class,
		// RealType.class, Iterable.class);

		// or 2. Cast it
		UnaryComputerOp mean_op = (UnaryComputerOp) ij.op().op(Ops.Stats.Mean.NAME,
			currentData.getImgPlus());

		Img<T> projection = (Img<T>) ij.op().transform().project(proj, currentData
			.getImgPlus(), mean_op, dim);
